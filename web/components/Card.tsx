export function Card({ kicker, title, desc, action, href, count }: { kicker: string; title: string; desc: string; action: string; href: string; count?: string }) {
  return (
    <div className="bg-[var(--bg)] border border-[var(--fg)] p-6 flex flex-col">
      ...
      <a href={href} className="mt-5 inline-block bg-[var(--fg)] text-[var(--bg)] px-4 py-2 text-xs font-bold text-center hover:bg-[var(--bg)] hover:text-[var(--fg)] border border-[var(--fg)] transition">{action}</a>
    </div>
  );
}
